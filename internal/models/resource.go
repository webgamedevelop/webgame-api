...
type IngressClass struct {
	gorm.Model
	Name      string `gorm:"type:varchar(50);uniqueIndex" binding:"required,max=50" form:"name" json:"name,omitempty"`
	ClassName string `gorm:"type:varchar(20)" binding:"required,max=20" form:"className" json:"className,omitempty"`
}

// ImagePullSecret k8s image pull secret
type ImagePullSecret struct {
	gorm.Model
	Name            string `gorm:"type:varchar(60);uniqueIndex" binding:"required,max=50" form:"name" json:"name,omitempty"`
...
type ResourceSpec struct {
	gorm.Model
	Name   string `gorm:"type:varchar(20);uniqueIndex" binding:"required,max=20" form:"name" json:"name,omitempty"`
	Cpu    string `gorm:"type:varchar(12)" binding:"required,max=12,k8sCpu" form:"cpu" json:"cpu,omitempty"`
	Memory string `gorm:"type:varchar(12)" binding:"required,max=12,k8sMemory" form:"memory" json:"memory,omitempty"`
}
...
type Repository struct {
	gorm.Model
	Name     string `gorm:"type:varchar(50);uniqueIndex" binding:"required,max=50" form:"name" json:"name,omitempty"`
	RepoName string `gorm:"type:varchar(120);uniqueIndex" binding:"required,max=120" form:"repoName" json:"repoName,omitempty"`
	Tags     []Tag  `json:"tags"`
}
...
type Tag struct {
	gorm.Model
	TagName      string `gorm:"type:varchar(100);uniqueIndex" binding:"required,max=100,semver" form:"tagName" json:"tagName,omitempty"`
	Comment      string `gorm:"type:varchar(200)" binding:"max=200" form:"comment" json:"comment,omitempty"`
